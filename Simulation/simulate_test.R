...
summary(results)
library(survival)
result <- clogit(Y~SNP15*SNP13+strata(fid), data = test)
summary(result)
lm(SNP15 ~ SNP13, data = test) %>% summary()
lm(SNP10 ~ SNP8, data = test[Y == 1,]) %>% summary()
lm(SNP10 ~ SNP8, data = test[Y == 0,]) %>% summary()
