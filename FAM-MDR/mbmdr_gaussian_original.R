...
MBMDR_o <- function(Y,EXPO,SNPS,ESTRAT,PVAL=0.1,dimen,first.model=NULL,output="mbmdr.out",AJUST=0,list.models=NULL,correction=T)
{
...
#    tab <- table(resp,exposed,exclude=NA)
    tab <- NULL
    if(correction & (any(tab==0) | sum(dim(tab))<4)){
  ...
  repeat{
    ...
    #second step
    h.list <- part[(part[,(dimen+2)]>0 & part[,(dimen+5)]<=PVAL),(2:(1+dimen))]
    l.list <- part[(part[,(dimen+2)]<0 & part[,(dimen+5)]<=PVAL),(2:(1+dimen))]
    ...
    if(!is.na(l.list[1])){
      ...
      write.table(t(c(model,"L",N,l.regout)),file=output,append=T,row.names=F,col.names=F,sep=";")
    }
    ...
    if(!is.na(h.list[1])){
      ...
      write.table(t(c(model,"H",N,h.regout)),file=output,append=T,row.names=F,col.names=F,sep=";")
    }
